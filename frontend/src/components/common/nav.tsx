...
<div className="flex  ">
<div className={` gap-6 text-sm hidden lg:flex border-black p-4  ${className} `}>
      {navLinks.map(link=>{
...
<div id="buttons" className="flex gap-4 py-2 justify-center items-center ">
...
{status?<button onClick={()=>setIsProfileClicked(prev=>!prev)
}><img src={Profile} alt="profile" className=" rounded-full  h-9 w-9 hover:contrast-50" /></button>:<Button name="Get started " className="" onClick={handleClick}/>}
</div>
